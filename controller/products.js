...
const createProducts = async (req, res) => {
    ...
    try {
        ...
        if (decoded.data.role == 'admin' || decoded.data.role == 'sub-admin' || decoded.data.role == 'inventory-admin') {
            const product = await Product.find({ productBrand: productBrand.toLowerCase().trim() });
            // const product_name = await Product.findOne({ productName });
            const productExist = product.some(function (prod) {
                return prod.productName.toLowerCase().trim() == productName.toLowerCase().trim();
            })
